...
BG_IMAGE_PATH = Path(__file__).parent / "assets" / "kelapa_sawit_bg.jpg"


@st.cache_data(show_spinner=False)
def load_bg_image_b64(path: Path):
    if not path.exists():
        return None
    return base64.b64encode(path.read_bytes()).decode("ascii")


_bg_b64 = load_bg_image_b64(BG_IMAGE_PATH)
_bg_css_layer = (
    f'url("data:image/jpeg;base64,{_bg_b64}")' if _bg_b64 else "none"
)
...
_dashboard_css = """
    <style>
      ...
      .stApp {
          background-image:
              linear-gradient(180deg, rgba(15,25,18,0.18) 0%, rgba(15,25,18,0.22) 45%, rgba(15,25,18,0.55) 100%),
              __BG_IMAGE_LAYER__ !important;
...
_dashboard_css = _dashboard_css.replace("__BG_IMAGE_LAYER__", _bg_css_layer)
